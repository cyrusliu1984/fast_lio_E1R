...
void set_posestamp(T &out)
{
    ...
    auto q_ = Eigen::Quaterniond(rob_rot.matrix());
    out.pose.orientation.x = q_.coeffs()[0];
    out.pose.orientation.y = q_.coeffs()[1];
    out.pose.orientation.z = q_.coeffs()[2];
    out.pose.orientation.w = q_.coeffs()[3];
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
    ...
    // 将Eigen旋转矩阵转换为tf::Quaternion
    Eigen::Quaterniond eigen_quat(rotation_matrix);
    tf::Quaternion tf_quat;
    tf_quat.setW(eigen_quat.w());
    tf_quat.setX(eigen_quat.x());
    tf_quat.setY(eigen_quat.y());
    tf_quat.setZ(eigen_quat.z());
    
    // 创建并发布camera_init到robot的变换
    static tf::TransformBroadcaster br;
    tf::Transform camera_to_robot;
    camera_to_robot.setOrigin(tf::Vector3(0, 0, 0));  // 原点位置不变
    camera_to_robot.setRotation(tf_quat);  // 应用旋转
    ...
    // 保持原有的里程计消息设置
    odomAftMapped.header.frame_id = "robot";  // 修改为新的父坐标系
    ...
    // 设置单位四元数，表示无旋转
    tf::Quaternion identity_rotation;
    identity_rotation.getIdentity();
    transform.setRotation(identity_rotation);
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            // 将旋转矩阵转换为SO3对象并应用到机器人姿态
            rob_rot = state_point.rot * Sophus::SO3(rotation_matrix);
            pos_lid = state_point.pos + state_point.rot.matrix() * state_point.offset_T_L_I;
